...
const drawboard = () => {
	...
	for (let y = start; y < size; y++) {
		for (let x = start; x < size; x++) {
			image += coords[x+','+y] !== undefined ? coords[x+','+y] : ' ';
		}
		image += "\n";
	}
	console.log(image);
...
let coords = {}, occupied = 0;
let is_x = true;
let start = 1000, size = 2500;
let ship_size = 99;
let x_start = 0;

const backtrace = (x, y) => {
	if (coords[x+','+y] !== 1) return false;
	if (coords[(x+ship_size)+','+y] !== 1) return false;
	if (coords[(x)+','+(y-ship_size)] !== 1) return false;
	if (coords[(x+ship_size)+','+(y-ship_size)] !== 1) return false;
	size = y+ship_size;
	for (let dx = x; dx <= x+ship_size; dx++) {
		for (let dy = y; dy >= y-ship_size; dy--) {
			coords[dx+','+dy] = 'X';
		}
	}
	drawboard();
	console.log('WooHoo!!!', x, y-ship_size); 
	process.exit();
	return true;
}
...
for (let y = start; y < size; y++) {
	let seen = 0;
	for (let x = x_start; x < size && seen < 2; x++) {
		...
		while (true) {
			if (program.return < 0 || program.return >= 90) {
				...
				if (seen === 0 && program.output === 1) { seen = 1; x_start = x; }
				if (seen === 1 && program.output === 0) { seen = 2; backtrace(x_start, y); }
				if (program.output) ++occupied;
				coords[x+','+y] = program.output;
				program.resume();
			} else if (program.return === 10) {
				const answer = is_x ? x : y;
				is_x = !is_x
